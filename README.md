...
Two-player Reversi (Othello) for the browser. A panther and a snow leopard take the
two sides: whoever's turn it is sits up alert, the other one dozes, and taking a
corner gets a roar.
...
The player who created the table owns the game state. The joiner sends move
intents and renders whatever comes back, and the host validates every incoming
move against the rules before applying it, so the two screens cannot drift apart
or be desynchronised by a malformed message.

## Running it locally

Open `index.html` in a browser. The **By code** tab works from a `file://` page.
The **Online** tab needs an `https://` origin, which is why it is served from
GitHub Pages rather than opened as a file.
